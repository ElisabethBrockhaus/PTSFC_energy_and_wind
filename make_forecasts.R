...
wind_data_raw_recent <- read_delim(paste0(data_dir, "stundenwerte_FF_04177_akt/produkt_ff_stunde_20210814_20230214_04177.txt")) %>%
...
wind_raw_years_plot <- ggplot() +
  ...
  scale_x_datetime() +
  coord_cartesian(xlim = c(as_datetime("2019-02-15"), as_datetime("2023-02-15")),
                  ylim = c(0,68), expand=F) +
  labs(x = NULL, y = "wind speed [km/h]")
wind_raw_weeks_plot <- ggplot(wind_data_raw_recent, aes(x=obs_tm, y=obs)) +
  geom_line() +
  scale_x_datetime() +
  coord_cartesian(xlim = c(as_datetime("2023-01-18"), as_datetime("2023-02-15")),
                  ylim=c(0,37), expand=F) +
  labs(x = NULL, y = "wind speed [km/h]")
wind_raw_plot <- ggarrange(wind_raw_years_plot, wind_raw_weeks_plot,
                             ncol=1, nrow=2, labels = list("A", "B"))
print(wind_raw_plot)
