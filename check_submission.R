...
dat <- read.csv(paste0("submissions/", today, "_PhoebeBuffay.csv"), stringsAsFactors = FALSE)
...
if(!all(dat$target %in% c("DAX", "energy", "wind"))){
  stop("target can only contain values 'DAX', 'energy' and 'wind'")
}else{
...
# check whether either all NA or all available
if(all(is.na(sub_wind[, cols_quantiles]))){
  cat("Target 'Wind' not submitted...")
}else{
  if(any(is.na(sub_wind[, cols_quantiles]))){
    warning("Target 'Wind' seems to be submitted, but contains some NA or non-numeric values.")
  }else{
    cat("checked: Submission complete for target 'Wind'.")
  }
}

# plausibility of wind values:
if(any(sub_wind[, cols_quantiles] > 300 | sub_wind[, cols_quantiles] < 0, na.rm = TRUE)){
